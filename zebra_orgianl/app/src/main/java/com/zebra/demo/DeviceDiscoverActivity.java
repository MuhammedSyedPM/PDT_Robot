...
import com.zebra.demo.rfidreader.reader_connection.ScanConnectTask;
...
public class DeviceDiscoverActivity extends BaseActivity implements Readers.RFIDReaderEventHandler, ResponseHandlerInterfaces.ReaderDeviceFoundHandler, ResponseHandlerInterfaces.BatteryNotificationHandler, ScannerAppEngine.IScannerAppEngineDevConnectionsDelegate {
    ...
    private static boolean usbSSIDWCheckInit = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        handleScannerCoexistenceOrInit();
        ...
        if(requestCode == BLUETOOTH_PERMISSION_REQUEST_CODE){
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                if(Application.sdkHandler.isSSIDwInterfaceCDCScannerAvailable()){
                    if(!usbSSIDWCheckInit) {
                        //usbSSIDWCheckInit = true;
                        showCoexistenceDialog(() -> startSfCoexistenceInit(false));
                    } else {
                        startSfCoexistenceInit(false);
                    }
                } else {
                    initialize();
                }
    ...
    private void initialize() {
        ...
        if(usbManager.getDeviceList().size() > 0) {
            UsbDevice usbRfidDevice = null;
            for (UsbDevice device : usbManager.getDeviceList().values()) {
                if ((device.getVendorId() == vendorId) && (device.getProductId() == productId)  || (device.getVendorId() == vendorId) && (device.getProductId() == 0x1702)) {
                    usbRfidDevice = device;
                    break;
                }
            }
            if (usbRfidDevice != null) {
                if (!usbManager.hasPermission(usbRfidDevice)) {
                    Intent intent = new Intent(INTENT_ACTION_GRANT_USB);
                    intent.setPackage(getPackageName());
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                        permissionIntent = PendingIntent.getBroadcast(this, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_MUTABLE);
                    } else {
                        permissionIntent = PendingIntent.getBroadcast(this, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT);
                    }
                    ...
                    usbManager.requestPermission(usbRfidDevice, permissionIntent);
                } else {
                    onPostPermissionGranted();
                }
            } else {
    ...
    private void initializeDcsSdk(boolean enableBTConnect){
        ...
        if(!Build.MODEL.startsWith("ET65") && !Build.MODEL.startsWith("ET60")) {
    ...
    private void forwardReaderAppearedToFragment(ReaderDevice readerDevice) {
    ...
    private void handleScannerCoexistenceOrInit() {
        if (Application.sdkHandler.isSSIDwInterfaceCDCScannerAvailable()) {
            showCoexistenceDialog(() -> startSfCoexistenceInit(true));
        } else {
            checkBtPermissionsThenInit(true);
        }
    }

    private void showCoexistenceDialog(Runnable onProceed) {
        View view = getLayoutInflater().inflate(R.layout.dialog_sf_dw_coexist_warning, null);
        new AlertDialog.Builder(this)
                .setView(view)
                .setCancelable(false)
                .setPositiveButton("OK", (d, w) -> {
                    usbSSIDWCheckInit = true;
                    Toast.makeText(this, "Setting co-existence mode...", Toast.LENGTH_SHORT).show();
                    onProceed.run();
                })
                .show();
    }

    private void startSfCoexistenceInit(boolean requestBTPermission) {
        Application.sdkHandler.setSfCoexistenceModeInit(result -> {
            if (result == SF_COEXISTENCE_SUCCESS || result == SF_COEXISTENCE_NOT_SUPPORTED) {
                checkBtPermissionsThenInit(requestBTPermission);
            } else {
                Log.e(TAG, "SF Co-existence failed");
                Toast.makeText(this, "SF Co-existence failed. Readers cannot be listed", Toast.LENGTH_SHORT).show();
            }
        });
    }

    private void checkBtPermissionsThenInit( boolean requestPermission ){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.S){
            if (requestPermission && ContextCompat.checkSelfPermission(this,
    ...
    private void setCoexistenceAndForward(ReaderDevice readerDevice) {
        Application.sdkHandler.setSfCoexistenceModeInit(result -> {
            if (result == SF_COEXISTENCE_SUCCESS || result == SF_COEXISTENCE_NOT_SUPPORTED) {
                ...
                //SF_COEXISTENCE_FAILURE
                runOnUi(() -> Toast.makeText(this,
