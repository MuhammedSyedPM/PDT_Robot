...
import com.zebra.demo.rfidreader.settings.BatteryFragementFXP20;
...
import com.zebra.demo.rfidreader.settings.ProfilesFragmentFXP20;
import com.zebra.demo.rfidreader.settings.adminlogin.AdminChangePasswordFragment;
import com.zebra.demo.rfidreader.settings.adminlogin.AdminLoginFragment;
import com.zebra.demo.rfidreader.settings.endpoint.EndpointConfigFragment;
import com.zebra.demo.rfidreader.settings.endpoint.EndpointConfigSettingsFragment;
...
import com.zebra.demo.rfidreader.settings.certificates.ReaderCertificatesFragment;
...
import com.zebra.demo.rfidreader.settings.endpoint.EndpointMainPage;
import com.zebra.demo.rfidreader.settings.endpoint.EndpointStatusFragment;
...
import static com.zebra.demo.rfidreader.settings.SettingsDetailActivity.REGULATORY_UPDATE_FAILURE;
...
import static com.zebra.demo.scanner.helpers.ActiveDeviceAdapter.ENDPOINT_STATUS_PAGE;
...
import static com.zebra.demo.scanner.helpers.ActiveDeviceAdapter.READER_WIFI_CERTIFICATE_TAB;
import static com.zebra.demo.scanner.helpers.ActiveDeviceAdapter.WIFI_STATUS_TAB;
...
public class ActiveDeviceActivity extends BaseActivity implements AdvancedOptionItemFragment.OnAdvancedListFragmentInteractionListener, ActionBar.TabListener, ScannerAppEngine.IScannerAppEngineDevEventsDelegate, ScannerAppEngine.IScannerAppEngineDevConnectionsDelegate,
    ...
    private String TAG = "ActiveDeviceActivity";
    ...
    static CommandExecutionTask  cmdExecTask = null;
    ...
    private ScannerFinder scannerFinder;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(mConnectedReader != null) {
            initializeView();
            scannerFinder = new ScannerFinder(this);
        } else {
        ...
        if(mConnectedReader != null && mConnectedReader.ReaderCapabilities != null) {
            short[] allAntennas = new short[mConnectedReader.ReaderCapabilities.getNumAntennaSupported()];
    ...
    protected void onResume() {
        ...
        // Perform any necessary checks asynchronously before showing the dialog
        new Handler().post(new Runnable() {
            @Override
            public void run() {
                if (waitingForFWReboot) {
                    // Ensure this dialog initialization is lightweight
                    showFwRebootdialog();
                }

                if((onSaveInstanceState) && (mReaderDisappeared != null) && (!RFIDController.regionNotSet)) {
                    //loadNextFragment(MAIN_HOME_SETTINGS_TAB);
                ...
                if(RFIDController.regionNotSet) {
                    launchSettingsScreen();
                }else if(Application.updateReaderConnection) {
    ...
    protected void onDestroy() {
        ...
        if (cmdExecTask != null) {
            cmdExecTask.shutdown();
        }
        if (scannerFinder != null) {
            scannerFinder.shutdown();
        }
    }
    ...
    public void onBackPressed() {
        ...
        switch(position) {
            ...
            case READERS_TAB:
                if (fragment instanceof PairOperationsFragment || fragment instanceof ReaderDetailsFragment) {
                    loadNextFragment(READER_LIST_TAB);
                } else {
                    minimizeApp();
                }
            ...
            case SETTINGS_TAB:
                ...
                if (fragment instanceof ManagerFragment) {
                    ...
                    loadNextFragment(MAIN_HOME_SETTINGS_TAB);
                        ...
                        || (fragment instanceof ProfileFragmentRE40)
                        || (fragment instanceof ProfilesFragmentFXP20)
                        || (fragment instanceof BatteryFragementFXP20)
                ...
                }else if(fragment instanceof ReaderWifiSettingsFragment){
                    loadNextFragment(WIFI_MAIN_PAGE);
                }else if(fragment instanceof WiFiMainPage){
                    loadNextFragment(MAIN_HOME_SETTINGS_TAB);
                }else if(fragment instanceof ReaderCertificatesFragment){
                    loadNextFragment(MAIN_HOME_SETTINGS_TAB);
                }else if (fragment instanceof EndpointConfigFragment) {
                    loadNextFragment(ENDPOINT_MAIN_PAGE);
                }else if (fragment instanceof EndpointConfigSettingsFragment) {
                    loadNextFragment(ENDPOINT_CONFIGURATION_PAGE);
                }else if (fragment instanceof EndpointStatusFragment) {
                    loadNextFragment(ENDPOINT_MAIN_PAGE);
                }else if (fragment instanceof EndpointMainPage) {
                    loadNextFragment(MAIN_HOME_SETTINGS_TAB);
                }else if (fragment instanceof AdminLoginFragment) {
                    loadNextFragment(MAIN_HOME_SETTINGS_TAB);
                }else if (fragment instanceof AdminChangePasswordFragment) {
                    loadNextFragment(ADMIN_LOGIN_PAGE);
                }
    ...
    public void startFirmware(View view) {
        ...
        cmdExecTask = new CommandExecutionTask(this);
        cmdExecTask.executeCommandTask(scannerID, DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_START_NEW_FIRMWARE, null,in_xml);
    ...
    public void abortFirmware(View view) {
        ...
        cmdExecTask = new CommandExecutionTask(this);
        cmdExecTask.executeCommandTask(scannerID, DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_ABORT_UPDATE_FIRMWARE, null,in_xml);
        //cmdExecTask.execute(new String[]{in_xml});
    }
    ...
    public void wifiStatusClicked( View view ){
        loadNextFragment(WIFI_STATUS_TAB);
    }

    public void wifiConfigPageClicked( View view ){
    ...
    public void wifiCertificatePageClicked( View view ){
        if(RFIDController.mConnectedReader != null && RFIDController.mConnectedReader.Config!=null) {
            int tab = RFD_DEVICE_MODE == DEVICE_STD_MODE ? SCAN_TAB:SETTINGS_TAB;
            setCurrentTabFocus(tab);
            loadNextFragment(READER_WIFI_CERTIFICATE_TAB);
        } else {
            Toast.makeText(this,"Reader Disappeared\nPlease try again later",Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public void wifiSettingsClicked( View view ){
    ...
    public void endpointConfigClicked(View view){
        if(RFIDController.mConnectedReader != null && RFIDController.mConnectedReader.Config!=null) {
            loadNextFragment(ENDPOINT_CONFIGURATION_PAGE);
        } else {
            Toast.makeText(this,"Reader Disappeared\nPlease try again later",Toast.LENGTH_SHORT).show();
    ...
    public void addNewEpClicked(View view){
        RFIDController.editEndpointPos = -1; //Reset the edit Endpoint Position
        loadNextFragment(ENDPOINT_CONFIG_SETTINGS_PAGE);
    }
    ...
    public void keyRemapClicked(View view) {
        if( RFIDController.mConnectedReader != null ) {
            ...
            ||  RFIDController.mConnectedReader.getHostName().startsWith("TC22R")
            || RFIDController.mConnectedReader.getHostName().startsWith("TC27R")) {
    ...
    public void endpointStatusClicked( View view ){
        loadNextFragment(ENDPOINT_STATUS_PAGE);
    }

    public void endpointMainPageClicked( View view ){
        loadNextFragment(ENDPOINT_MAIN_PAGE);
    }

    public void adminConnectClicked(View v){
        loadNextFragment(ADMIN_LOGIN_PAGE);
    }
    ...
    public void enableScanning(View view) {
        ...
        cmdExecTask.execute(new String[]{in_xml});*/
        cmdExecTask = new CommandExecutionTask(this);
        cmdExecTask.executeCommandTask(scannerID, DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_DEVICE_SCAN_ENABLE, null,in_xml);
    ...
    public void disableScanning(View view) {
       ...
       /* cmdExecTask = new MyAsyncTask(scannerID, DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_DEVICE_SCAN_DISABLE, null);
        cmdExecTask.execute(new String[]{in_xml});*/
        cmdExecTask = new CommandExecutionTask(this);
        cmdExecTask.executeCommandTask(scannerID, DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_DEVICE_SCAN_DISABLE, null,in_xml);
    ...
    public void aimOn(View view) {
     ...
     /*   cmdExecTask = new MyAsyncTask(scannerID, DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_DEVICE_AIM_ON, null);
        cmdExecTask.execute(new String[]{in_xml});*/
        cmdExecTask = new CommandExecutionTask(this);
        cmdExecTask.executeCommandTask(scannerID, DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_DEVICE_AIM_ON, null,in_xml);
    ...
    public void aimOff(View view) {
       ...
       /* cmdExecTask = new MyAsyncTask(scannerID, DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_DEVICE_AIM_OFF, null);
        cmdExecTask.execute(new String[]{in_xml});*/
        cmdExecTask = new CommandExecutionTask(this);
        cmdExecTask.executeCommandTask(scannerID, DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_DEVICE_AIM_OFF, null,in_xml);
    ...
    public void pullTrigger(View view) {
       ...
       /* cmdExecTask = new MyAsyncTask(scannerID, DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_DEVICE_PULL_TRIGGER, null);
        cmdExecTask.execute(new String[]{in_xml});*/
        cmdExecTask = new CommandExecutionTask(this);
        cmdExecTask.executeCommandTask(scannerID, DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_DEVICE_PULL_TRIGGER, null,in_xml);
    ...
    public void releaseTrigger(View view) {
        ...
        cmdExecTask.execute(new String[]{in_xml});*/
        cmdExecTask = new CommandExecutionTask(this);
        cmdExecTask.executeCommandTask(scannerID, DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_DEVICE_RELEASE_TRIGGER, null,in_xml);
    ...
    public void scanTrigger(View view) {
        ...
        cmdExecTask.execute(new String[]{in_xml});*/
        cmdExecTask = new CommandExecutionTask(this);
        cmdExecTask.executeCommandTask(Application.currentConnectedScannerID, DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_DEVICE_PULL_TRIGGER, null,in_xml);
...
//                startActivity(detailsIntent);
                    ...
                    try {
                        ...
                        if(mConnectedDevice != null && mConnectedReader != null) {
                            if (mConnectedDevice.getDeviceCapability(mConnectedReader.getHostName()).contains("WiFi")) {
                                if(SaveConfigurationsFragment.saveWifiConfig()){
                                    RFIDResults result = mConnectedReader.Config.wifiSaveProfile();
                                    if (RFIDResults.RFID_API_SUCCESS != result) {
                                        bResult = false;
                                    }
                                }
                            }
                    ...
                    if (!result) {
                        if(operationFailureException!= null )
                         Toast.makeText(getApplicationContext(), operationFailureException.getVendorMessage(), Toast.LENGTH_SHORT).show();
                            ...
                            Toast.makeText(getApplicationContext(), "save config not supported", Toast.LENGTH_SHORT).show();
                    } else
    ...
    public void issueReaderDetached(ReaderDevice connectedDevice) {
        mRFIDBaseActivity.RFIDReaderDisappeared(connectedDevice);
    }

    public void issueReaderAttached(ReaderDevice connectedDevice) {
        mRFIDBaseActivity.RFIDReaderAppeared(connectedDevice);
    }
    ...
    public void setPickListMode(int picklistInt) {
        ...
        cmdExecTask.execute(new String[]{in_xml});*/
        cmdExecTask = new CommandExecutionTask(this);
        cmdExecTask.executeCommandTask(scannerID, DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_RSM_ATTR_SET, outXML,in_xml);
    ...
    public void loadBatteryStatistics(View view) {
        ...
        BatteryCheckTask batteryCheckTask = new BatteryCheckTask();
        batteryCheckTask.executeBatteryCheck(scannerID, DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_RSM_ATTR_GETALL, this, BatteryStatistics.class);
    }
    /**
    ...
    public void loadScanSpeedAnalytics(View view) {
        ...
        if (SsaSetSymbologyActivity.SSA_SYMBOLOGY_ENABLED_FLAG) {
           ...
           // new AsyncTaskSSASvailable(scannerID, DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_RSM_ATTR_GETALL, this, SsaSetSymbologyActivity.class).execute(new String[]{in_xml});
            SSAAvailabilityTask ssaTask = new SSAAvailabilityTask();
            ssaTask.executeSSAAvailabilityCheck(scannerID, DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_RSM_ATTR_GETALL, this, SsaSetSymbologyActivity.class, in_xml);
        }
    }
    ...
    public class BatteryCheckTask {
        ...
        public void executeBatteryCheck(final int scannerId, final DCSSDKDefs.DCSSDK_COMMAND_OPCODE opcode, final Context context, final Class<?> targetClass) {
            CustomProgressDialog progressDialog = new CustomProgressDialog(context, "Please wait...");
            progressDialog.setCancelable(false);
            progressDialog.show();

            Future<Boolean> future = executorService.submit(() -> {
                boolean result = performBatteryCheck(scannerId, opcode);

                Intent intent = new Intent(context, targetClass);
                intent.putExtra(Constants.SCANNER_ID, scannerId);
                intent.putExtra(Constants.SCANNER_NAME, getIntent().getStringExtra(Constants.SCANNER_NAME));
                intent.putExtra(Constants.BATTERY_STATUS, result);
            ...
            executorService.submit(() -> {
                try {
                    Boolean result = future.get(); // This will block if needed
                    // Handle the result if necessary
                } catch (Exception e) {
                    Log.e("BatteryCheckTask", "Error executing battery check", e);
                } finally {
                    progressDialog.dismiss();
                }
            });
        ...
        private Boolean performBatteryCheck(int scannerId, DCSSDKDefs.DCSSDK_COMMAND_OPCODE opcode) {
            StringBuilder sb = new StringBuilder();
            String in_xml = "<inArgs><scannerID>" + scannerID + "</scannerID></inArgs>";
            boolean result = executeCommand(opcode, in_xml, sb, scannerId);
            if (opcode == DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_RSM_ATTR_GETALL && result) {
                try {
                    XmlPullParser parser = Xml.newPullParser();
                    ...
                    Log.e("BatteryCheckTask", e.toString());
                }
            }
            return false;
        }

       /* // You should implement this method based on your command execution logic
        private boolean executeCommand(DCSSDKDefs.DCSSDK_COMMAND_OPCODE opcode, String command, StringBuilder sb, int scannerId) {
    ...
    public class SSAAvailabilityTask {

        private static final String TAG = "SSAAvailabilityTask";
        private final ExecutorService executorService = Executors.newSingleThreadExecutor();
        private ArrayList<Integer> ssaSupportedAttribs = new ArrayList<>();

        public void executeSSAAvailabilityCheck(int scannerId, DCSSDKDefs.DCSSDK_COMMAND_OPCODE opcode, Context context, Class<?> targetClass, String command) {
            CustomProgressDialog progressDialog = new CustomProgressDialog(context, "Please wait...");
            progressDialog.setCancelable(false);
            progressDialog.show();

            Callable<Boolean> task = () -> performSSAAvailabilityCheck(scannerId, opcode, command);

            Future<Boolean> future = executorService.submit(task);

            executorService.submit(() -> {
                try {
                    Boolean result = future.get(); // Wait for the task to complete
                    handleResult(context, targetClass, scannerId, result);
                } catch (Exception e) {
                    Log.e(TAG, "Error executing SSA availability check", e);
                } finally {
                    progressDialog.dismiss();
                }
            });
        ...
        private Boolean performSSAAvailabilityCheck(int scannerId, DCSSDKDefs.DCSSDK_COMMAND_OPCODE opcode, String command) {
            StringBuilder sb = new StringBuilder();
            boolean result = executeCommand(opcode, command, sb, scannerId);
            if (opcode == DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_RSM_ATTR_GETALL && result) {
                try {
                    XmlPullParser parser = Xml.newPullParser();
                    parser.setInput(new StringReader(sb.toString()));
        ...
        private void handleResult(Context context, Class<?> targetClass, int scannerId, Boolean result) {
            ...
            Intent intent = new Intent(context, targetClass);
            intent.putExtra(Constants.SCANNER_ID, scannerId);
            intent.putIntegerArrayListExtra(Constants.SYMBOLOGY_SSA, ssaSupportedAttribs);
            intent.putExtra(Constants.SSA_STATUS, ssaStatus);
            context.startActivity(intent);
        }
    }
    ...
    public void findScanner(View view) {
       ...
       // new FindScannerTask(scannerID).execute();
        scannerFinder.findScanner(scannerID);
    }
    ...
    public class CommandExecutionTask {
        ...
        public CommandExecutionTask(Context context) {
            this.context = context;
        }

        public void executeCommandTask(int scannerId, DCSSDKDefs.DCSSDK_COMMAND_OPCODE opcode, StringBuilder outXML, String command) {
            CustomProgressDialog progressDialog = new CustomProgressDialog(context, "Execute Command...");
            progressDialog.show();

            Future<Boolean> future = executorService.submit(() -> {
                return executeCommand(opcode, command, outXML, scannerId);
            });
        ...
        private void handleResult(Boolean result) {
            if (!result) {
                showToast("Cannot perform the action");
            }
        }
    ...
    public class ScannerFinder {
        ...
        private static final String TAG = "ScannerFinder";
        private final ExecutorService executorService = Executors.newSingleThreadExecutor();
        private final Context context;

        public ScannerFinder(Context context) {
            this.context = context;
        }

        public void findScanner(int scannerId) {
            if (btnFindScanner != null) {
                btnFindScanner.setEnabled(false);
            }
            ...
            executorService.submit(() -> {
                try {
                    ...
                    Log.e(TAG, "Error finding scanner", e);
                }
            });
        }
    ...
    public synchronized void showAntennaSelection(View view) {
        if(mIsInventoryRunning) {
            Toast.makeText(getApplicationContext(), "Operation in progress-command not allowed", Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    public void loadNextFragment(int fragmentType) {
        ...
        Log.d(TAG,"settingsTab=>"+settingsTab);
        String PageTitle= " " ;
     try {
         ...
         switch (fragmentType) {
             case RAPID_READ_TAB:
                 ...
                 getSupportFragmentManager().beginTransaction().remove(getCurrentFragment(RFID_TAB)).commitAllowingStateLoss();
                 break;
             case LOCATE_TAG_TAB:
                 PageTitle = "Locate";
             ...
             case RFID_SETTINGS_TAB:
                 ...
                 getSupportFragmentManager().beginTransaction().remove(getCurrentFragment(settingsTab)).commitAllowingStateLoss();
                 break;
             ...
             case SCAN_DATAVIEW_TAB:
                 ...
                 getSupportFragmentManager().beginTransaction().remove(getCurrentFragment(SCAN_TAB)).commitAllowingStateLoss();
                 break;
             ...
             case READER_LIST_TAB:
                 ...
                 getSupportFragmentManager().beginTransaction().remove(getCurrentFragment(READERS_TAB)).commitAllowingStateLoss();
                 break;
             ...
             case READER_WIFI_SETTINGS_TAB:
                 ...
                 getSupportFragmentManager().beginTransaction().remove(getCurrentFragment(settingsTab)).commitAllowingStateLoss();
                 break;
             case READER_WIFI_CERTIFICATE_TAB:
                 PageTitle = "Certificates";
                 mAdapter.setSettingsMode(READER_WIFI_CERTIFICATE_TAB);
                 getSupportFragmentManager().beginTransaction().remove(getCurrentFragment(settingsTab)).commitAllowingStateLoss();
                 break;
             ...
             case WIFI_MAIN_PAGE:
                 ...
                 getSupportFragmentManager().beginTransaction().remove(getCurrentFragment(settingsTab)).commitAllowingStateLoss();
                 break;
             case ENDPOINT_CONFIGURATION_PAGE:
                 PageTitle = "Endpoint Configuration";
                 mAdapter.setSettingsMode(ENDPOINT_CONFIGURATION_PAGE);
                 getSupportFragmentManager().beginTransaction().remove(getCurrentFragment(settingsTab)).commitAllowingStateLoss();
                 break;
             case ENDPOINT_CONFIG_SETTINGS_PAGE:
                 PageTitle = "Endpoint Settings";
                 mAdapter.setSettingsMode(ENDPOINT_CONFIG_SETTINGS_PAGE);
                 getSupportFragmentManager().beginTransaction().remove(getCurrentFragment(settingsTab)).commitAllowingStateLoss();
                 break;
             case ENDPOINT_STATUS_PAGE:
                 PageTitle = "Endpoint Status";
                 mAdapter.setSettingsMode(ENDPOINT_STATUS_PAGE);
                 getSupportFragmentManager().beginTransaction().remove(getCurrentFragment(settingsTab)).commitAllowingStateLoss();
                 break;
             case ENDPOINT_MAIN_PAGE:
                 PageTitle = "Endpoint Main";
                 mAdapter.setSettingsMode(ENDPOINT_MAIN_PAGE);
                 getSupportFragmentManager().beginTransaction().remove(getCurrentFragment(settingsTab)).commitAllowingStateLoss();
                 break;
             case ADMIN_LOGIN_PAGE:
                 PageTitle = "Admin Login";
                 mAdapter.setSettingsMode(ADMIN_LOGIN_PAGE);
                 getSupportFragmentManager().beginTransaction().remove(getCurrentFragment(settingsTab)).commitAllowingStateLoss();
                 break;
             case ADMIN_CHANGE_PASSWORD_PAGE:
                PageTitle = "Change Admin Password";
                mAdapter.setSettingsMode(ADMIN_CHANGE_PASSWORD_PAGE);
                getSupportFragmentManager().beginTransaction().remove(getCurrentFragment(settingsTab)).commitAllowingStateLoss();
                break;
         }
    ...
    ActivityResultLauncher<Intent> settingsDetailLauncher = registerForActivityResult(

            new ActivityResultContracts.StartActivityForResult(),
            new ActivityResultCallback<ActivityResult>() {
                @Override
                public void onActivityResult(ActivityResult result) {
                    if(result.getResultCode() == REGULATORY_UPDATE_FAILURE) {
                        Log.d(TAG,"Setting Screen Cancelled the request");
                        Intent intent = new Intent(getApplicationContext(), DeviceDiscoverActivity.class);
                        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
                        startActivity(intent);
                        finish();
                    }
    ...
    public void launchSettingsScreen() {
        ...
        settingsDetailLauncher.launch(detailsIntent);
    }
