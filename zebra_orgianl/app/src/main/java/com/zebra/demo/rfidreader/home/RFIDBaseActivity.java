...
import com.zebra.demo.rfidreader.settings.endpoint.IotEventData;
import com.zebra.demo.rfidreader.settings.endpoint.IotEventManager;
...
import static com.zebra.demo.rfidreader.rfid.RFIDController.fingerRestMode;
...
import static com.zebra.rfid.api3.GenericUtil.isCharmanderDevice;
import static com.zebra.scannercontrol.SDKHandler.SfCoexistenceResultListener.SfCoexistenceResult.SF_COEXISTENCE_NOT_SUPPORTED;
import static com.zebra.scannercontrol.SDKHandler.SfCoexistenceResultListener.SfCoexistenceResult.SF_COEXISTENCE_SUCCESS;
...
public class RFIDBaseActivity implements Readers.RFIDReaderEventHandler,
    ...
    private boolean isMIdleModeChangeRecvrRegistered = false;
    private boolean ismDeviceIdleRecverRegistered = false;
...
//         PP+ Battery support
    ...
    private void disableBatterySaverModeChangeListner(){
        ...
        if(mIdlemodechangereciever != null && isMIdleModeChangeRecvrRegistered) {
            mActivity.unregisterReceiver(mIdlemodechangereciever);
            isMIdleModeChangeRecvrRegistered = false;
        }
        if(mDeviceIdlereceiver != null && ismDeviceIdleRecverRegistered) {
            mActivity.unregisterReceiver(mDeviceIdlereceiver);
            ismDeviceIdleRecverRegistered = false;
        }
    }
    private void enableBatterySaverModeChangeListner() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            ismDeviceIdleRecverRegistered = true;
            ...
            isMIdleModeChangeRecvrRegistered = true;
        }
    }
...
private boolean isMReceiverRegistered = false;
    ...
    private void initializeConnectionSettings() {
        ...
        fingerRestMode = settings.getBoolean(Constants.FINGER_REST_MODE, false);
    ...
    public void LoadProfiles() {
        ...
        if(modelName.contains("FXP20")){
            ProfilesFragmentFXP20 contentFXP20 = new ProfilesFragmentFXP20();
        }
    ...
    public void onDestroy() {
      ...
      if(mReceiver!=null && isMReceiverRegistered) {
          mActivity.unregisterReceiver(mReceiver);
          isMReceiverRegistered = false;
      }
...
//            ((AccessOperationsFragment) fragment).deviceConnected(device);
    ...
    public void RFIDReaderAppeared(ReaderDevice readerDevice) {
        Log.d(TAG, "ReaderAppeared");

        if (Application.sdkHandler.isSSIDwInterfaceCDCScannerAvailable()) {
            runOnUi(() -> showCoexistenceDialog(() -> setCoexistenceAndForward(readerDevice)));
        } else {
            runOnUi(() -> forwardReaderAppearedToFragment(readerDevice));
        }
    }

    private void forwardReaderAppearedToFragment(ReaderDevice device) {
    ...
    private void showCoexistenceDialog(Runnable onProceed) {
        View view = mActivity.getLayoutInflater().inflate(R.layout.dialog_sf_dw_coexist_warning, null);
        new androidx.appcompat.app.AlertDialog.Builder(mActivity)
                .setView(view)
                .setCancelable(false)
                .setPositiveButton("OK", (d, w) -> {
                    Toast.makeText(mActivity, "Setting co-existence mode...", Toast.LENGTH_LONG).show();
                    onProceed.run();
                })
                .show();
    }

    private void setCoexistenceAndForward(ReaderDevice readerDevice) {
        Application.sdkHandler.setSfCoexistenceModeInit(result -> {
            if (result == SF_COEXISTENCE_SUCCESS || result == SF_COEXISTENCE_NOT_SUPPORTED) {
                runOnUi(() -> forwardReaderAppearedToFragment(readerDevice));
            } else {
                runOnUi(() -> Toast.makeText(mActivity.getApplicationContext(),
                        "SF Co-existence failed. Reader cannot be listed",
    ...
    * On factory reset clean up
    ...
    private ReaderDevice getReaderDevice(ReaderDevice readerdevice) {
        ...
        try {
            if( mConnectedReader.getHostName().startsWith("RFID") || isCharmanderDevice() || mConnectedReader.getHostName().contains("FXP20")) {
    ...
    private void notificationFromGenericReader(RfidStatusEvents rfidStatusEvents) {
        ...
        if (rfidStatusEvents.StatusEventData.getStatusEventType() == STATUS_EVENT_TYPE.DISCONNECTION_EVENT) {
            ...
            Boolean triggerRealesd = false;
            if (rfidStatusEvents.StatusEventData.HandheldTriggerEventData.getHandheldEvent() == HANDHELD_TRIGGER_EVENT_TYPE.HANDHELD_TRIGGER_PRESSED)
                triggerPressed = true;
            if (rfidStatusEvents.StatusEventData.HandheldTriggerEventData.getHandheldEvent() == HANDHELD_TRIGGER_EVENT_TYPE.HANDHELD_TRIGGER_RELEASED)
                triggerRealesd = true;
            if (rfidStatusEvents.StatusEventData.HandheldTriggerEventData.getHandheldEvent() == HANDHELD_TRIGGER_EVENT_TYPE.HANDHELD_TRIGGER_UNLOCK){
                triggerRealesd = true;
                RFIDController.isTriggerLocked = false;}
            if (rfidStatusEvents.StatusEventData.HandheldTriggerEventData.getHandheldEvent() == HANDHELD_TRIGGER_EVENT_TYPE.HANDHELD_TRIGGER_LOCK)
                RFIDController.isTriggerLocked = true;
            ...
            if (m_ScreenOn) {
                ...
                } else if (triggerRealesd && isTriggerImmediateorRepeat(triggerPressed) && fragment instanceof TriggerEventHandler) {
        ...
        else if (rfidStatusEvents.StatusEventData.getStatusEventType() == STATUS_EVENT_TYPE.IOT_EVENT_MGMT) {
            handleIotEvent(
                    rfidStatusEvents.StatusEventData.iotEventMgmt.getCause(),
                    rfidStatusEvents.StatusEventData.iotEventMgmt.getEpType(),
                    rfidStatusEvents.StatusEventData.iotEventMgmt.getEpName(),
                    rfidStatusEvents.StatusEventData.iotEventMgmt.getStatus(),
                    rfidStatusEvents.StatusEventData.iotEventMgmt.getReason()
            );
        } else if (rfidStatusEvents.StatusEventData.getStatusEventType() == STATUS_EVENT_TYPE.IOT_EVENT_MGMT_EVT) {
            handleIotEvent(
                    rfidStatusEvents.StatusEventData.iotEventMgmtEvt.getCause(),
                    rfidStatusEvents.StatusEventData.iotEventMgmtEvt.getEpType(),
                    rfidStatusEvents.StatusEventData.iotEventMgmtEvt.getEpName(),
                    rfidStatusEvents.StatusEventData.iotEventMgmtEvt.getStatus(),
                    rfidStatusEvents.StatusEventData.iotEventMgmtEvt.getReason()
            );
        } else if (rfidStatusEvents.StatusEventData.getStatusEventType() == STATUS_EVENT_TYPE.IOT_EVENT_CONTROL) {
            handleIotEvent(
                    rfidStatusEvents.StatusEventData.iotEventControl.getCause(),
                    rfidStatusEvents.StatusEventData.iotEventControl.getEpType(),
                    rfidStatusEvents.StatusEventData.iotEventControl.getEpName(),
                    rfidStatusEvents.StatusEventData.iotEventControl.getStatus(),
                    rfidStatusEvents.StatusEventData.iotEventControl.getReason()
            );
        } else if (rfidStatusEvents.StatusEventData.getStatusEventType() == STATUS_EVENT_TYPE.IOT_EVENT_DATA1) {
            handleIotEvent(
                    rfidStatusEvents.StatusEventData.iotEventData1.getCause(),
                    rfidStatusEvents.StatusEventData.iotEventData1.getEpType(),
                    rfidStatusEvents.StatusEventData.iotEventData1.getEpName(),
                    rfidStatusEvents.StatusEventData.iotEventData1.getStatus(),
                    rfidStatusEvents.StatusEventData.iotEventData1.getReason()
            );
        } else if (rfidStatusEvents.StatusEventData.getStatusEventType() == STATUS_EVENT_TYPE.IOT_EVENT_DATA2) {
            handleIotEvent(
                    rfidStatusEvents.StatusEventData.iotEventData2.getCause(),
                    rfidStatusEvents.StatusEventData.iotEventData2.getEpType(),
                    rfidStatusEvents.StatusEventData.iotEventData2.getEpName(),
                    rfidStatusEvents.StatusEventData.iotEventData2.getStatus(),
                    rfidStatusEvents.StatusEventData.iotEventData2.getReason()
            );
        }
        else if(rfidStatusEvents.StatusEventData.getStatusEventType() == STATUS_EVENT_TYPE.CERTIFICATE_EVENT) {
            String certEvent = rfidStatusEvents.StatusEventData.certificateEventData.getStatus();
            Log.d(TAG, "notificationFromGenericReader "+" CERTIFICATE_EVENT : " + certEvent);
            Fragment fragment1 =  mActivity.getCurrentFragment(SETTINGS_TAB);
            mActivity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    if (fragment1 instanceof ReaderCertificatesFragment) {
                        ((ReaderCertificatesFragment) fragment1).handleCertificateEvent(certEvent);
                    }
                }
            });
        }
        else if(rfidStatusEvents.StatusEventData.getStatusEventType() == STATUS_EVENT_TYPE.CONNECTED_INTERFACE_EVENT) {
            String connectedInterfaceType = rfidStatusEvents.StatusEventData.connectedInterfaceEventData.getInterface();
            Log.d(TAG, "notificationFromGenericReader[connectedInterfaceType] :" + connectedInterfaceType);
            RFIDController.mConnectedReader.ReaderCapabilities.setConnectedInterfaceType(connectedInterfaceType);
        }

    }

    private void handleIotEvent(String cause, String epType, String epName, String status, String reason) {
        Log.d(TAG, "STATUS_EVENT_TYPE.IOT_EVENT Cause: " + cause + ", EpType: " + epType + ", EpName: " + epName + ", Status: " + status + ", Reason: " + reason);

        IotEventData iotEventData = new IotEventData();
        iotEventData.setCause(cause);
        iotEventData.setEpType(epType);
        iotEventData.setEpName(epName);
        iotEventData.setStatus(status);
        iotEventData.setReason(reason);

        IotEventManager.getInstance().notifyEvent(iotEventData);
    }
                    ...
                    if (isAccessCriteriaRead) {
                        if (accessTagCount == 0) {
                            ...
                            if(command.equals("Lock"))
                                sendNotification(Constants.ACTION_READER_STATUS_OBTAINED, mActivity.getString(R.string.err_access_lock_op_failed));
                            if(command.equals("Kill"))
                                sendNotification(Constants.ACTION_READER_STATUS_OBTAINED, mActivity.getString(R.string.err_access_kill_op_failed));
                        }
                        isAccessCriteriaRead = false;
                    } else {
