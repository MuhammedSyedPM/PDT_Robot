    ...
    private void fetchDeviceInfo()
    {
        if(RFIDController.mConnectedReader != null) {
            try {
                ...
                String connectedInterfaceType =  RFIDController.mConnectedReader.ReaderCapabilities.getConnectedInterfaceType();
                ...
                if(RFIDController.mConnectedReader.getHostName().startsWith("RFID")){
                    pcfw = RFIDController.mConnectedReader.ReaderCapabilities.getFirwareVersion();
                    nge = RFIDController.mConnectedReader.ReaderCapabilities.getFirwareVersion();
                    ModelName =  RFIDController.mConnectedReader.getHostName().substring(4);
                    SerialNo =  RFIDController.mConnectedReader.ReaderCapabilities.getSerialNumber();
                }
                if(RFIDController.mConnectedReader.getHostName().contains("FXP20")){
                    pcfw = RFIDController.mConnectedReader.ReaderCapabilities.getFirwareVersion();
                    nge = RFIDController.mConnectedReader.ReaderCapabilities.getFirwareVersion();
                }
                ...
                TextView ConnInterfTyperTV= (TextView) ((ActiveDeviceActivity)getActivity()) .findViewById(R.id.ConnectionTypeTextView);
                if(!connectedInterfaceType.equals("") || !connectedInterfaceType.isEmpty()) {
                    ConnInterfTyperTV.setText(connectedInterfaceType);
                }else{
                    TableRow entry  = (TableRow) ((ActiveDeviceActivity)getActivity()) .findViewById(R.id.connectiontypetablerowid);
                    entry.setVisibility(View.GONE);
