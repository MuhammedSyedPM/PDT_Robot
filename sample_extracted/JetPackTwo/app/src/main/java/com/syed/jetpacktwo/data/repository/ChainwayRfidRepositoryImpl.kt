...
class ChainwayRfidRepositoryImpl @Inject constructor(
    ...
    override fun connectToReader(scannerSpec: String) {
        ...
        scope.launch {
            try {
                ...
                if (result) {
                    val version = mReader?.version ?: "Unknown"
                    Log.d("ChainwayRepo", "Init Success. Module Version: $version")
                    // Set default power to 30
                    mReader?.setPower(30)
                    // Set American/Global Frequency (4) if not set
                    // mReader?.setFrequencyMode(4) 
                    _readerStatus.value = ReaderStatus("Chainway Connected ($version)", Color.GREEN, true, false)
                } else {
            ...
            reader.setInventoryCallback(object : IUHFInventoryCallback {
                override fun callback(info: UHFTAGInfo?) {
                    ...
                    info?.let { tag ->
                        ...
                        if (epc.isNotEmpty() && !rfidDataCaptured.contains(epc)) {
    ...
    override fun startTagTracing(epc: String) {
        ...
        mReader?.let { reader ->
            ...
            val result = reader.startLocation(context, epc, 1, 32, object : IUHFLocationCallback {
                override fun getLocationValue(value: Int, valid: Boolean) {
                    if (valid) {
                        scope.launch {
                            _traceData.emit(TracedTagInfo(epcHex = epc, scaledRssi = value))
                        }
                    }
                }
            })
