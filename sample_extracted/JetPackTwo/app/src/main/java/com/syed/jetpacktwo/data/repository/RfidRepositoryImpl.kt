...
class RfidRepositoryImpl @Inject constructor(
    ...
    override fun getTraceData(): Flow<TracedTagInfo> = callbackFlow {
        ...
        controller.setListener(object : TraceTagController.TraceTagListener {
            override fun traceTagEvent(data: TraceTagController.TracedTagInfo) {
                ...
                trySend(TracedTagInfo(epcHex = epcHex, scaledRssi = data.scaledRssi))
            }
    ...
    override fun disconnectedEvent() {
        ...
        mainHandler.post { showConnecting(hAcTr?.address ?: "") }
    }
                ...
                for (x in mTagsAddedCounter until mTagsAddedCounter + event.tagsAdded) {
                    ...
                    if (readingEnabled) {
                        _tagReadEvents.tryEmit(
                            TagReadEvent(epcString, mTagsAddedCounter + event.tagsAdded)
                        )
                    }
                }
                mTagsAddedCounter += event.tagsAdded
            } catch (ex: Exception) {
                updateStatus("RFID read error: ${ex.message}", Color.RED, _readerStatus.value.isConnected)
            }
        }
    }
